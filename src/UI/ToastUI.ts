...
export class ToastUI {
  ...
  public show(
    ...
    try {
      ...
      console.warn("Toast 표시 중 오류:", error);
    }
  }
  ...
  private createToastContainer(
    ...
    // 배경 (둥근 모서리와 그림자 효과)
    const background = this.createToastBackground(
      backgroundWidth,
      backgroundHeight,
      type
    );
  ...
  private showWithAnimation(
    ...
    try {
      ...
      console.warn("Toast 애니메이션 설정 중 오류:", error);
    }
  }
  ...
  private animateIn(
    ...
    const animate = (currentTime: number): void => {
      // toast가 유효하지 않으면 애니메이션 중단
      if (!toast || !toast.parent || toast.destroyed) {
        return;
      }
      ...
      try {
        switch (position) {
          ...
          case "bottom":
            toast.y = this.lerp(
              position === "top" ? -toast.height : this.screenHeight,
              targetY,
              easedProgress
            );
        ...
        console.warn("Toast 애니메이션 중 오류:", error);
        // 오류 발생 시 애니메이션 중단
      }
    };
  ...
  private animateOut(toast: PIXI.Container): void {
    ...
    const animate = (currentTime: number): void => {
      ...
      try {
        ...
        console.warn("Toast 아웃 애니메이션 중 오류:", error);
        // 오류 발생 시 강제로 정리
        try {
          if (toast.parent) {
            this.container.removeChild(toast);
          ...
          console.warn("Toast 정리 중 오류:", cleanupError);
        }
      }
    };
  ...
  public getInfo(): {
    ...
    if (this.currentToast && this.currentToast.children.length > 1) {
      const textChild = this.currentToast.children[1] as PIXI.Text;
      currentMessage = textChild.text;
    }
